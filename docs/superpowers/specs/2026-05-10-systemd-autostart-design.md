# Kaizen systemd auto-start (Pi boot)
...
Make Kaizen start automatically on Pi 5 boot so the device behaves like a finished appliance: power on, wait for the wake word, talk. No manual `./run.sh --voice` from a shell.
...
User-level (vs. system-level) was chosen because the Pi's audio stack runs in the user session: PipeWire-pulse + the SoundCloud/Spotify clients all expect `XDG_RUNTIME_DIR` and the user's PulseAudio socket. Running Kaizen inside the same user session avoids the runtime-dir/audio-group workarounds a system-level unit would need.
...
4. The user manager activates `kaizen.service` (it's in `default.target`'s wants).
5. The unit's `ExecStart` runs `~/kaizen/run.sh --voice`.
...
### 1. Unit file — `config/systemd/kaizen.service`
...
3. Copy `config/systemd/kaizen.service` → `~/.config/systemd/user/kaizen.service`.
...
7. `systemctl --user enable --now kaizen.service`.
8. `systemctl --user --no-pager status kaizen.service` and print the journalctl one-liner: `journalctl --user -u kaizen -f`.
...
1. `systemctl --user disable --now kaizen.service` (ignore "not loaded" errors).
2. `rm -f ~/.config/systemd/user/kaizen.service`.
...
- Day-to-day: `systemctl --user {start,stop,restart,status} kaizen`.
- Live logs: `journalctl --user -u kaizen -f`.
- Crash investigation: `journalctl --user -u kaizen --since '1 hour ago' -p err`.
...
| Failure | Handled by |
|---------|------------|
| USB mic/DAC not yet enumerated at first start | `Restart=on-failure` + 5s delay; Kaizen fails fast on device-resolve, retries until USB is up |
...
| User wants to stop the service | `systemctl --user stop kaizen` — won't auto-resurrect (Restart=on-failure only) |
| Pi reboots during a turn | Cold start path takes over on next boot |

## Testing

- **Local (dev workstation):** install the unit on the dev box (without `enable-linger`); confirm `systemctl --user start kaizen` brings up the text or voice loop, `stop` halts it cleanly, and a forced `kill -9` triggers the 5s restart.
- **Pi (real hardware):** run the installer, reboot, confirm Kaizen responds to the wake word with no shell intervention. Pull the USB mic mid-run and confirm the journal records the error and systemd retries.
- **Crash logging:** force a Python exception (e.g. temporarily raise in `main.py` startup), reboot, confirm `journalctl --user -u kaizen -p err` shows the traceback.
...
- **Linger has side effects.** Enabling linger means *every* user service is kept alive across logout, not just Kaizen. Acceptable on a dedicated Pi appliance.
