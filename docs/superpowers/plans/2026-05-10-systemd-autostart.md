# MiniClaw systemd auto-start Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a user-level systemd unit + installer/uninstaller scripts so MiniClaw starts on Pi boot and survives crashes.

**Architecture:** One unit file (`config/systemd/miniclaw.service`) checked into the repo as source of truth. A `scripts/install_systemd_service.sh` script copies it into `~/.config/systemd/user/`, ensures `loginctl enable-linger` and persistent journal, then enables the unit. A symmetric uninstaller. README gets a "Run on boot" section.
...
| File | Purpose |
|------|---------|
| `config/systemd/miniclaw.service` (new) | Source-of-truth unit file. Uses `%h` so it's user-portable. |
...
**Files:**
- Create: `config/systemd/miniclaw.service`
...
Run: `cd ~/linux/miniclaw && .venv/bin/python -m pytest tests/test_systemd_unit.py -v`
Expected: FAIL — `FileNotFoundError: ... miniclaw.service`.
...
```bash
git add config/systemd/miniclaw.service tests/test_systemd_unit.py
git commit -m "feat(systemd): add miniclaw.service user unit"
```
...
git commit -m "feat(systemd): add idempotent install script for miniclaw.service"
```
...
git commit -m "feat(systemd): add uninstall script for miniclaw.service"
```
...
```bash
ssh pi "cd ~/miniclaw && ./scripts/install_systemd_service.sh"
```
...
ssh pi "systemctl --user status miniclaw --no-pager"
```
...
sleep 6
ssh pi "systemctl --user status miniclaw --no-pager"
ssh pi "journalctl --user -u miniclaw --since '1 minute ago' | tail -30"
```
...
```bash
ssh pi "systemctl --user stop miniclaw"
sleep 10
ssh pi "systemctl --user status miniclaw --no-pager"
```
...
```bash
ssh pi "systemctl --user start miniclaw"
```
...
- Pi reboots → MiniClaw responds to the wake word with no shell intervention (Task 5, Step 4).
